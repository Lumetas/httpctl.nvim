...
function M._create_buf_with_win(bufname, output_window_split)
	...
	if not M.move_to_win then
		vim.api.nvim_set_current_win(current_win)
	end

	-- activate the window
	if M.move_to_win then
		print("move_to_win")
...
function M.new(config)
	local cfg = vim.tbl_deep_extend("force", {
		...
		response = { auto_focus_response = true, output_window_split = "right" },
	}, config or {})

	M.move_to_win = cfg.response.auto_focus_response

	local out = setmetatable({
		cfg = cfg,
		bufname = cfg.bufname or default_bufname,
		output_window_split = cfg.response.output_window_split,
		auto_focus_response = cfg.response.auto_focus_response,
		current_menu_id = 0,
